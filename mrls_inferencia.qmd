...
-   Como combinações lineares de variáveis normais são normais, os estimadores $\hat\beta_0$ e $\hat\beta_1$, que são combinações lineares dos $Y_i$, passam a ter distribuições normais exatas em amostras finitas [@montgomery2021].

-   A estatística baseada na soma dos quadrados dos resíduos passa a ter distribuição qui-quadrado, o que permite derivar distribuições $t$ e $F$ de forma exata [@kutner2005].
...
Esse resultado decorre da decomposição ortogonal do vetor $Y$ em componente ajustada e componente residual, cuja demonstração também pode ser consultada no Apêndice {#demo} [@kutner2005]. A perda de dois graus de liberdade reflete a estimação dos dois parâmetros $\beta_0$ e $\beta_1$.
...
## Estimação pontual da média condicional e de uma nova observação individual

A predição pontual constitui o primeiro passo para compreender como o modelo de regressão descreve a relação entre $Y$ e $X$. Antes de introduzir intervalos, é fundamental reconhecer que as quantidades estimadas no modelo não são fixas: elas dependem da amostra observada e, portanto, são **variáveis aleatórias**. Sob a hipótese de normalidade dos erros, essas quantidades possuem distribuição conhecida, o que viabiliza a construção de inferência.

### Distribuição dos valores ajustados 

Para um valor fixo $X_0$, definimos a média condicional estimada por

$$
\hat{\mu}(X_0) \;=\; \hat\beta_0 + \hat\beta_1 X_0.
$$

Como os estimadores $\hat\beta_0$ e $\hat\beta_1$ são combinações lineares das observações $Y_i$, segue que $\hat{\mu}(X_0)$ também é uma combinação linear dessas variáveis. Assim, sob erros normais, $\hat{\mu}(X_0)$ possui distribuição normal.
...
mostrando que $\hat{\mu}(X_0)$ é um estimador **não viesado** da média condicional. Sua variância é dada por
...
Essa expressão evidencia um aspecto importante da regressão: a incerteza associada à média estimada é menor na vizinhança de $\bar X$ e cresce à medida que $X_0$ se afasta do centro dos dados, refletindo a concentração de informação amostral.


### Predição pontual da resposta

Considere agora o problema de prever o valor de uma nova observação no ponto $X_0$. Nesse caso, o modelo gerador pode ser escrito como
...
em que $\varepsilon_{\text{novo}} \sim N(0,\sigma^2)$ é independente dos erros da amostra utilizada para ajustar o modelo.

A predição pontual natural para essa nova observação é

$$
\hat{Y}_0 = \hat{\mu}(X_0).
$$

Entretanto, diferentemente do caso anterior, agora o interesse recai sobre uma **realização individual** da variável resposta, e não apenas sobre sua média. Isso implica que a incerteza associada à predição deve considerar duas fontes distintas: a incerteza na estimação dos parâmetros e a variabilidade intrínseca do erro aleatório.

Para caracterizar essa incerteza, analisamos a variável

$$
Y_{\text{novo}}(X_0) - \hat{Y}_0.
$$

Como $Y_{\text{novo}}(X_0)$ e $\hat{Y}_0$ são combinações lineares de variáveis normais independentes, essa diferença também possui distribuição normal. Sua esperança é zero, e sua variância é

$$
Var\!\left[Y_{\text{novo}}(X_0) - \hat{Y}_0\right]
=
\sigma^2 +
\sigma^2\left[
\frac{1}{n} + \frac{(X_0-\bar X)^2}{S_{xx}}
...
De forma equivalente, isso implica que a própria predição possui distribuição normal,
...
O termo adicional "$+1$" na variância reflete a presença de um novo erro aleatório, inexistente no caso da média condicional. Esse resultado evidencia que a predição de uma observação individual é necessariamente mais incerta do que a estimação da média.


### Interpretação, implicações e limitações

A distinção entre média condicional e predição individual é fundamental. Enquanto $\hat{\mu}(X_0)$ busca estimar o comportamento médio do fenômeno, a variável $Y_{\text{novo}}(X_0)$ incorpora variações aleatórias que não podem ser eliminadas pelo modelo.

Essa diferença se traduz diretamente nas variâncias: a variância da predição individual é sempre maior, pois inclui tanto a incerteza da estimação quanto a variabilidade inerente ao processo gerador dos dados. Em termos práticos, isso significa que previsões individuais são naturalmente menos precisas do que estimativas da média.

A predição pontual fornece apenas um valor central e não expressa a incerteza associada ao processo. Por essa razão, sua utilização isolada é insuficiente em aplicações estatísticas.
...
Essa substituição tem consequência direta na forma das distribuições amostrais: ao padronizarmos os estimadores utilizando $s$ em vez de $\sigma$, as estatísticas resultantes deixam de seguir a normal padrão e passam a seguir distribuições $t$ de Student com $n-2$ graus de liberdade [@kutner2005; @montgomery2021].
...
segue distribuição $t$ de Student com $n-2$ graus de liberdade [@kutner2005; @montgomery2021]. A demonstração formal pode ser consultada no Apêndice de Demonstrações {#demo}.
...
Enquanto o intervalo de confiança está associado à **média condicional** $\mu(X_0)$, em muitas situações o interesse recai sobre a predição do valor de uma **nova observação individual** no ponto $X_0$.

Nesse contexto, a predição pontual continua sendo dada por $\hat{\mu}(X_0)$. No entanto, a incerteza associada a essa predição é maior, pois envolve não apenas a estimação dos parâmetros, mas também a variabilidade inerente a uma nova realização do erro aleatório.

De fato, conforme discutido anteriormente, a diferença

$$
Y_{\text{novo}}(X_0) - \hat{\mu}(X_0)
$$

segue, sob normalidade dos erros, uma distribuição normal com média zero e variância
...
Substituindo $\sigma$ por sua estimativa $s$ e padronizando essa quantidade, obtemos uma estatística que segue distribuição $t$ de Student com $n-2$ graus de liberdade. Isso conduz ao intervalo de predição de nível $(1-\alpha)\times 100\%$:
...
A diferença em relação ao intervalo para a média condicional é direta: surge o termo adicional $1$ dentro da raiz, que representa a **variabilidade individual irreducível** do processo.

Essa distinção tem implicações importantes. O intervalo de predição é necessariamente mais amplo, pois incorpora uma fonte extra de incerteza. Além disso, assim como no caso da média, sua largura aumenta à medida que $X_0$ se afasta de $\bar X$, refletindo a menor disponibilidade de informação nessa região.

Do ponto de vista interpretativo, o intervalo de predição descreve um conjunto de valores plausíveis para uma **nova observação individual**. Em termos frequentistas, se o processo fosse repetido nas mesmas condições, aproximadamente $(1-\alpha)\times 100\%$ desses intervalos conteriam o valor observado.

Em aplicações, os intervalos de confiança são apropriados para descrever o comportamento médio do fenômeno, enquanto intervalos de predição são utilizados quando o objetivo é antecipar resultados individuais.
