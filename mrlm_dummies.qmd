...
Essa observação é importante porque mostra que as variáveis fictícias não são um procedimento externo à regressão linear. Elas são parte natural da estrutura matricial do MRLM [@rencher2012; @seber2003].
...
Portanto, $\beta_0$ representa a média do grupo de referência, isto é, o grupo codificado com $D_i=0$. Já $\beta_1$ representa a diferença média entre o grupo codificado com $D_i=1$ e o grupo de referência [@montgomery2021; @greene2018].
...
modelo_paralelo <- lm(
  ...
  data = dados_iris_dummy
)

grid_pred <- expand.grid(
  Petal.Length = seq(
    min(dados_iris_dummy$Petal.Length),
    max(dados_iris_dummy$Petal.Length),
    length.out = 100
  ),
  Species = levels(dados_iris_dummy$Species)
)

grid_pred$Sepal_Length_pred <- predict(
  modelo_paralelo,
  newdata = grid_pred
)
...
Quando uma variável categórica possui apenas duas categorias, uma única dummy é suficiente. Entretanto, quando há $k$ categorias, usamos $k-1$ variáveis fictícias para evitar dependência linear perfeita entre as colunas da matriz de planejamento [@seber2003; @rencher2012].
...
Importante notar que a escolha da categoria de referência não altera o ajuste do modelo nem os valores ajustados. O que muda é a interpretação dos coeficientes. Por isso, a categoria de referência deve ser escolhida de modo a facilitar a interpretação.

Embora a codificação por categoria de referência seja a forma mais comum em regressão aplicada, ela não é a única maneira de representar uma variável categórica. A escolha da codificação depende do objetivo da análise e da interpretação desejada para os coeficientes. 

Na codificação por referência, uma categoria é omitida e passa a ser incorporada ao intercepto. Os coeficientes das demais categorias representam diferenças médias em relação a essa categoria. Essa forma é especialmente adequada quando existe um grupo de comparação natural, como um grupo controle em estudos experimentais, uma região de referência em estudos geográficos ou uma categoria padrão em aplicações econômicas e sociais.

Outra possibilidade é usar contrastes por soma. Nesse caso, os coeficientes são interpretados como desvios em relação à média geral, e não como diferenças em relação a uma categoria específica. Essa codificação pode ser útil quando nenhuma categoria deve ser tratada como referência natural ou quando o interesse está em comparar cada grupo com o comportamento médio global.

Também existem contrastes ortogonais, frequentemente utilizados em experimentos planejados e em análises de fatores com níveis ordenados ou estruturados. Nesses casos, os contrastes podem ser escolhidos para representar comparações específicas, como tendência linear, tendência quadrática ou diferenças entre grupos previamente definidos.

Portanto, a codificação escolhida não altera o espaço gerado pelas colunas associadas ao fator, nem os valores ajustados do modelo quando as parametrizações são equivalentes. O que muda é a forma como os coeficientes são apresentados e interpretados. Em aplicações introdutórias e em grande parte dos modelos aplicados, a codificação por referência é a mais direta; em análises experimentais ou em situações nas quais o interesse recai sobre comparações específicas entre grupos, outras codificações podem ser mais informativas.
...
Esse problema é conhecido como armadilha das variáveis fictícias, ou *dummy variable trap*. A solução usual é omitir uma categoria, que passa a ser a referência [@greene2018; @montgomery2021].
...
Um modelo de regressão contendo apenas variáveis fictícias para representar os níveis de um fator é equivalente a uma ANOVA de um fator [@montgomery2021; @seber2003].
...
Esse tipo de modelo é a base da ANCOVA, pois combina comparação entre grupos com ajuste por variáveis quantitativas [@montgomery2021].
...
modelo_paralelo <- lm(mpg ~ wt + am, data = dados_mtcars)
...
A centralização consiste em substituir $x_i$ por $x_i-\bar{x}$, tornando a interpretação do intercepto e dos efeitos principais mais informativa em modelos com interação [@montgomery2021; @greene2018].
...
Quando uma variável categórica possui várias categorias, é comum testar o efeito conjunto do fator por meio de um teste $F$ envolvendo simultaneamente os coeficientes das dummies associadas ao fator [@seber2003; @montgomery2021].
...
O uso de dummies, portanto, não cria uma nova teoria de estimação. Ele amplia a capacidade interpretativa do MRLM ao permitir que fatores qualitativos sejam incorporados à matriz de planejamento [@rencher2012; @seber2003].

## Ilustrações em R

A base `mtcars`, disponível no R, foi utilizada para ilustrar dummies binárias e interações. A variável `am` indica o tipo de transmissão do automóvel, sendo usualmente interpretada como automática ou manual.

A base `iris`, também disponível no R, foi usada para ilustrar fatores com múltiplas categorias. A variável `Species` identifica três espécies de flores.
