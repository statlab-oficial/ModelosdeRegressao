...
codifica variabilidade e dependência. 
a) O que representam os elementos da diagonal?\
...
$$ a) Identifique a média condicional e variância condicional.\
...
e $\hat{\boldsymbol{\beta}}$ como forma linear. 

a) Mostre que $\hat{\boldsymbol{\beta}}=(\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{X}^\top\mathbf{Y}$ é transformação linear de $\mathbf{Y}$.\
